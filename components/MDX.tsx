...
// Collapse nested anchors and add external attrs when needed
function Anchor(props: any) {
    const { href = "", children, className, ...rest } = props;
    const kids = React.Children.toArray(children);
    if (kids.length === 1 && React.isValidElement(kids[0]) && (kids[0] as any).type === "a") {
    ...
    return (
        <a
            href={href}
            className={cx("underline hover:no-underline", className)}
